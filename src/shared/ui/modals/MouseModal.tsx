...
export const MouseModal: FC<ModalProps> = ({icon, content, isOpen}) => {
  ...
  return (
    <div className={combineStyle([ModalStyle.CopyNumberModal, TextModule.span__light, isOpen ? ModalStyle['open'] : ModalStyle['closed']])} style={{left: mousePos.x, top: mousePos.y}}>
      {icon}
      {content}
    </div>
  );
